...
class ScenarioManagerNode(Node):
    def __init__(self):
        ...
        # 웹(/information)과의 호환: web_request_node와 동일한 토픽 구독
        self.create_subscription(String, '/information', self._on_information, 10)

        # robot4 언독 액션 클라이언트
        if _HAS_UNDOCK:
            self._undock_client = ActionClient(self, Undock, '/robot4/undock')
        else:
            self._undock_client = None
            self.get_logger().warn('irobot_create_msgs not found: robot4 undock disabled')
        ...
        self._interaction_robot: Optional[str] = None  # INTERACTING을 보낸 로봇 추적
        self._robot4_needs_undock = False   # robot4 최초 목적지 전 언독 필요 여부
        self._robot4_pending_pose = None    # 언독 대기 중인 goal pose
    ...
    def _on_information(self, msg: String):
        """웹 /information 토픽 호환: rosbridge envelope을 벗겨서 처리."""
        try:
            envelope = json.loads(msg.data)
        except json.JSONDecodeError:
            return
        # rosbridge publish envelope: {"op":"publish","topic":"/information","msg":{...}}
        inner = envelope.get('msg', envelope)
        if not isinstance(inner, dict):
            return
        inner_msg = String()
        inner_msg.data = json.dumps(inner)
        self._on_scenario_request(inner_msg)
    ...
    def _start_interaction(self, payload: dict):
        if self.state == 'IDLE':
            ...
            self._interaction_robot = request_robot
    ...
    def _start_escort(self, payload: dict):
        ...
        # origin_floor 자동 추론: 웹이 미전달 시 robot_id / 이전 INTERACTING 로봇으로 판단
        if 'origin_floor' not in payload and not (payload.get('origin') or {}).get('floor'):
            payload = dict(payload)
            req_robot = str(
                payload.get('robot_id') or self._interaction_robot or ''
            ).strip('/')
            if req_robot == ROBOT4:
                payload['origin_floor'] = 2
            elif self.state == 'IDLE':
                # INTERACTING 없이 직접 온 ESCORT: 목적지 층으로 출발층 추론 (2F→2F)
                dest_name = (
                    (payload.get('destination') or {}).get('poi_id')
                    or payload.get('destination_name')
                    or payload.get('poi_id')
                )
                dest_floor = int(
                    (LOCATIONS.get(dest_name) or {}).get('floor', 1)
                ) if dest_name else 1
                if dest_floor == 2:
                    payload['origin_floor'] = 2
    ...
    def _start_dispatch(self):
        ...
        # robot4가 목적지 로봇이면 최초 이동 전 언독 필요 (station2에 도킹 중일 수 있음)
        if self.destination_robot == ROBOT4:
            self._robot4_needs_undock = True
    ...
    def _send_goal(self, robot: str, pose, escort: bool):
        ...
        # robot4 첫 목적지: station2에 도킹 중일 수 있으므로 언독 먼저
        if robot == ROBOT4 and self._robot4_needs_undock:
            self._robot4_needs_undock = False
            self._robot4_pending_pose = pose
            if self._undock_client is not None and self._undock_client.wait_for_server(timeout_sec=1.0):
                future = self._undock_client.send_goal_async(Undock.Goal())
                future.add_done_callback(self._on_robot4_undock)
                return
            self.get_logger().warn('robot4 undock server unavailable, navigating directly')
            self._robot4_pending_pose = None
        self._publish_goal(robot, pose)

    def _publish_goal(self, robot: str, pose):
    ...
    def _on_robot4_undock(self, future):
        pose = self._robot4_pending_pose
        self._robot4_pending_pose = None
        if pose is None:
            return
        try:
            ...
            self.get_logger().warn(f'robot4 undock request error: {e}')
            self._publish_goal(ROBOT4, pose)
            return
        if not handle.accepted:
            self.get_logger().info('robot4 undock rejected (already undocked)')
            self._publish_goal(ROBOT4, pose)
            return
        self._robot4_pending_pose = pose
        handle.get_result_async().add_done_callback(self._on_robot4_undock_done)

    def _on_robot4_undock_done(self, _future):
        pose = self._robot4_pending_pose
        self._robot4_pending_pose = None
        if pose is not None:
            self._publish_goal(ROBOT4, pose)
    ...
    def _mark_done(self, robot: str):
        ...
        if self.robot_stage and all(stage == 'DONE' for stage in self.robot_stage.values()):
            ...
            # 순찰을 실제로 멈췄을 때만 resume 발행 (2F→2F는 순찰 안 멈춤)
            if self.patrol_is_stopped:
                self.resume_pubs[self.patrol_robot].publish(Empty())
            self._reset()
    ...
    def _scenario_uses_patrol_robot(self) -> bool:
        # 같은 층 이동(2F→2F 등)은 support_robot 미사용: destination_robot만 체크
        if self.plan is not None and self.plan.same_floor:
            return self.destination_robot == self.patrol_robot
    ...
    def _destination_route(self, robot: str):
        ...
        home_name = HOME.get(robot)
        # robot2(1F→1F)도 에스코트 완료 후 HOME(station)으로 복귀
        if home_name and (not route or route[-1][0] != home_name):
            route.append((home_name, False))
        return deque(route)
